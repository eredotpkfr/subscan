...
async fn get_next_url_test() {
    ...
    let next = AlienVault::get_next_url(url, Value::Null);
...
async fn extract_test() {
    ...
    let extracted = AlienVault::extract(json, TEST_DOMAIN.to_string());
    let not_extracted = AlienVault::extract(Value::Null, TEST_DOMAIN.to_string());
