...
async fn get_next_url_test() {
    ...
    let next = WhoisXMLAPI::get_next_url(url, Value::Null);
...
async fn extract_test() {
    ...
    let extracted = WhoisXMLAPI::extract(json, TEST_DOMAIN.to_string());
    let not_extracted = WhoisXMLAPI::extract(Value::Null, TEST_DOMAIN.to_string());
